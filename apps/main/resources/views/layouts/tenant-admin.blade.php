...
<head>
    ...
    <title>@yield('title', 'Dashboard') — {{ $tenant->name ?? 'Admin' }}</title>
...
<div class="d-flex" x-data="{ sidebarOpen: true }" :class="{ 'sidebar-collapsed': !sidebarOpen }">
    ...
    <aside class="sidebar d-flex flex-column">
        <div class="sidebar-brand d-flex align-items-center gap-2">
            @if($tenant->logo_url)
                <img src="{{ $tenant->logo_url }}" alt="{{ $tenant->name }}" style="height:28px; border-radius:4px;">
            @else
                <span>🏪</span>
            @endif
        ...
        <nav class="flex-grow-1 py-2">
            ...
            <a href="{{ route('tenant.admin.dashboard') }}" title="Dashboard"
               class="nav-link {{ request()->routeIs('tenant.admin.dashboard') ? 'active' : '' }}">
                <i class="bi bi-speedometer2"></i><span>Dashboard</span>
            </a>

            <div class="nav-section-label"><span>Konten</span></div>
            <a href="{{ route('tenant.admin.products.index') }}" title="Produk"
               class="nav-link {{ request()->routeIs('tenant.admin.products.*') ? 'active' : '' }}">
                <i class="bi bi-box-seam"></i><span>Produk</span>
            </a>

            <div class="nav-section-label"><span>Transaksi</span></div>
            <a href="{{ route('tenant.admin.bookings.index') }}" title="Booking"
               class="nav-link {{ request()->routeIs('tenant.admin.bookings.*') ? 'active' : '' }}">
                <i class="bi bi-ticket-perforated"></i><span>Booking</span>
            </a>
            <a href="{{ route('tenant.admin.invoices.index') }}" title="Invoice"
               class="nav-link {{ request()->routeIs('tenant.admin.invoices.*') ? 'active' : '' }}">
        ...
        <div class="sidebar-footer" x-data="{ profileOpen: false }" @click.outside="profileOpen = false">
            ...
            <div class="profile-dropdown" x-show="profileOpen" x-cloak>
                ...
                <a href="{{ route('tenant.admin.profile.show') }}" class="profile-dropdown-item">
    ...
    <div class="main-content d-flex flex-column">
        ...
        <div class="topbar d-flex align-items-center gap-3">
            ...
            <div class="ms-auto d-flex align-items-center gap-2">
                <span class="badge bg-primary">{{ Auth::guard('admin_session')->user()?->role?->label() }}</span>
            </div>
        </div>
