...
<div class="container py-5" style="max-width:960px">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{ route('activities.index') }}" class="btn btn-outline-secondary btn-sm px-4">
            Kembali ke Beranda
        </a>
    </div>
    ...
    <div class="text-center py-5">
        ...
        <a href="{{ route('activities.index') }}" class="btn btn-primary mt-2">Lihat Aktivitas</a>
    </div>
    @else
                    ...
                    <div class="p-4 {{ !$loop->last ? 'border-bottom' : '' }}">
                        <div class="d-flex justify-content-between align-items-start gap-3">
                            ...
                            <div class="flex-grow-1 min-width-0">
                                ...
                                @if($item['pax_adult'] > 0)
                                <div class="small text-muted">{{ $item['pax_adult'] }} X ADULT</div>
                                @endif
                                @if($item['pax_child'] > 0)
                                <div class="small text-muted">{{ $item['pax_child'] }} X CHILD</div>
                                @endif
                                ...
                                {{-- Delete button --}}
                                <form method="POST"
                                      action="{{ route('cart.remove', $item['index']) }}"
                                      class="mt-3 d-inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-between text-muted small mb-2">
                        <span>Sub Total ({{ $items->count() }} {{ $items->count() === 1 ? 'barang' : 'barang' }})</span>
